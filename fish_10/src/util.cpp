...
std::string HttpDownloadTextFile(const std::wstring& a_url)
{
	...
	if (hInet)
	{
		...
		if (hRequest)
		{
			...
			if (true)
			{
				DWORD dContentLength = 0;
				DWORD dContentLengthVarSize = sizeof(dContentLength);

				if (::HttpQueryInfo(hRequest, HTTP_QUERY_CONTENT_LENGTH | HTTP_QUERY_FLAG_NUMBER, &dContentLength, &dContentLengthVarSize, nullptr) == TRUE)
				{
					uFileSize = dContentLength;
				}
			}
